...
public class ArticleServiceImpl implements IArticleService {
    ...
    // 统计前10的热度文章信息
    @Override
    public List<Article> getHeatArticles(int count) {
        PageHelper.startPage(1,count);
        List<Article> articleList = articleMapper.selectArticleWithPage();
        for (int i = 0; i < articleList.size(); i++) {
            Article article = articleList.get(i);
            List<Statistic> statistic = statisticMapper.getStatistic();
            article.setHits(statistic.get(i).getHits());
            article.setCommentsNum(statistic.get(i).getCommentsNum());
        }
        return articleList;
    }
